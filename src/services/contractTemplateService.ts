...
export const STANDARD_LEGAL_TEMPLATES: LawyerContractTemplate[] = [
  {
    id: 'std_main_contract_simple',
    title: '개인회생·파산 사건 위임계약서 (실무 간략 박스형 - 추천)',
    description: '로펌 실무에서 널리 쓰이는 수임료 간략 요약 박스(말일 분납, 2회 미납 시 폐지, 인가까지 추가비용 없음)가 적용된 심플 계약서',
    category: 'contract',
    type: 'main_contract',
    signatureRequired: 'both',
    requiredConfirmationText: '수임료 분납 조건 및 면책/인가 보증 특약을 확인하였습니다',
    tags: ['실무추천', '간략박스형', '위임계약', '도산특화'],
    createdAt: '2026-01-01T00:00:00.000Z',
    updatedAt: '2026-01-01T00:00:00.000Z',
    content: `개인회생·파산 사건 위임 계약서 (실무 간략형)
...
제 5 조 (수임료)
① 갑은 을에게 수임료로 총 금 {{총수임료한글}} 원({{부가세여부}})[일금 {{총수임료}}]을 지급한다.
┌────────────────────────────────────────────────────────┐
│ - {{수임료분납약정}}
...
export function applyTemplatePlaceholders(content: string, vars: PlaceholderVariables): string {
  ...
  return content
    ...
    .replace(/\{\{총수임료한글\}\}/g, totalFeeKorean)
    .replace(/\{\{부가세여부\}\}/g, vatStatus)
    .replace(/\{\{수임료분납약정\}\}/g, feeScheduleText)
