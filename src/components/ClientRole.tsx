      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-6">
        ...
        {activeTab === 'lawyers' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            {/* Grid of Lawyers with Pagination */}
            {(() => {
              ...
              const paginatedLawyers = filteredLawyers.slice((lawyerPage - 1) * itemsPerPage, lawyerPage * itemsPerPage);

              return (
                <div className="space-y-6">
                  {/* Total Count Info */}
                  <div className="text-left text-xs text-slate-500 dark:text-slate-400 font-medium">
                    총 <span className="text-brand dark:text-brand-light font-bold">{filteredLawyers.length}</span>명의 도산 전문 변호사가 활동 중입니다.
                  </div>
                            ...
                            <div className="flex-1 space-y-2">
                              ...
                              <div className="pt-2 flex items-center justify-between text-xs border-t border-slate-100 dark:border-slate-800">
                                ...
                                <button 
                                  ...
                                  className="bg-brand hover:bg-brand text-white font-semibold px-3 py-1.5 rounded-lg transition-colors text-[11px] cursor-pointer"
                                >
                                  상담 신청하기
                                </button>
                              </div>
                      ...
                      {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-1.5 pt-4">
                          <button
                            ...
                            onClick={() => {
                              setLawyerPage(prev => Math.max(1, prev - 1));
                              window.scrollTo({ top: 0, behavior: 'smooth' });
                            }}
                          ...
                          {Array.from({ length: totalPages }).map((_, idx) => {
                            const pNum = idx + 1;
                            const isCurrent = lawyerPage === pNum;
                            return (
                              <button
                                key={pNum}
                                type="button"
                          ...
                          <button
                            type="button"
                            disabled={lawyerPage === totalPages}
                            onClick={() => {
                              setLawyerPage(prev => Math.min(totalPages, prev + 1));
                              window.scrollTo({ top: 0, behavior: 'smooth' });
                            }}
                            className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                              lawyerPage === totalPages
                              ? 'bg-slate-50 dark:bg-slate-950 border-slate-100 dark:border-slate-850 text-slate-400 cursor-not-allowed'
                              : 'bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-350'
                            }`}
                          >
        ...
        {activeTab === 'chat' && (
          <div className="max-w-7xl mx-auto space-y-6 animate-fadeIn">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (matchedRequest) {
                          ...
                          setActiveTab('request');
                          alert('제안서의 변호사님과 대화를 나누시려면, 먼저 간단 자가진단 상담 신청을 완료해 주세요! (1초 완료)');
                          return;
                        }
                        setActiveTab('chat');
                        window.scrollTo({ top: 0, behavior: 'smooth' });
            ...
            {/* Chat Workspace Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 bg-white dark:bg-slate-900 rounded-xl overflow-hidden shadow-lg border border-slate-200 dark:border-slate-800 min-h-[500px] h-[calc(100vh-14rem)] lg:h-[650px]">
