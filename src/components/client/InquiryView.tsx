    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fadeIn text-left pb-12">
      ...
      {!isLoggedIn ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-3xl p-8 text-center space-y-4">
          ...
          <button onClick={onShowAuthModal} className="bg-brand hover:bg-brand-dark text-white font-extrabold px-6 py-2.5 rounded-[200px] text-xs transition-all shadow-md cursor-pointer">간편 가명 로그인 진행하기</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <form onSubmit={handleCreateInquiry} className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-sm">
            <h3 className="font-extrabold text-sm text-slate-800 dark:text-white flex items-center gap-1.5 pb-2 border-b border-slate-100 dark:border-slate-800"><span>🙋</span><span>새로운 문의 등록하기</span></h3>
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <textarea rows={6} value={inquiryContent} onChange={(e) => setInquiryContent(e.target.value)} placeholder="질문 내용을 자세히 기재해 주시면 관리자가 성심성의껏 답변드리겠습니다." className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 text-slate-700 dark:text-slate-200 font-normal leading-relaxed text-xs focus:outline-none focus:ring-1 focus:ring-brand" />
            </div>
            <button type="submit" className="w-full bg-brand hover:bg-brand-dark text-white font-extrabold py-2.5 rounded-[200px] text-xs transition-colors cursor-pointer text-center">문의 제출하기 (Stealth 전송)</button>
          </form>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-sm">
            <h3 className="font-extrabold text-sm text-slate-800 dark:text-white flex items-center gap-1.5 pb-2 border-b border-slate-100 dark:border-slate-800"><span>📋</span><span>나의 문의 내역 및 관리자 답변</span></h3>
              ...
              {myInquiries.map(inq => (
                <div key={inq.id} className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-150 dark:border-slate-855 space-y-3.5">
                  <div className="flex justify-between items-center">
                    <span className={`text-[9px] px-2 py-0.5 rounded border font-extrabold ${inq.status === 'replied' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20'}`}>{inq.status === 'replied' ? '답변 완료' : '답변 대기'}</span>
                  ...
                  {inq.replyContent ? (
                    ...
                    <div className="text-center py-2 text-[10px] text-slate-400 font-bold bg-slate-100/50 dark:bg-slate-900/30 rounded-xl">⏰ 관리자가 질문을 확인하고 답변을 작성 중입니다. 잠시만 기다려 주세요.</div>
                  )}
                </div>
              ))}
