...
export default function AuthModal({ onClose, onLoginSuccess }: AuthModalProps) {
  ...
  const handleSocialLogin = async (provider: string) => {
    ...
    onLoginSuccess(generatedAlias, `${provider.toLowerCase()}@social.login`, channel as any);
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white dark:bg-slate-900 border-t sm:border border-slate-200 dark:border-slate-800 rounded-t-3xl sm:rounded-3xl max-w-full sm:max-w-md w-full shadow-2xl p-6 md:p-8 space-y-4 relative overflow-hidden text-left animate-slideUp sm:animate-fadeIn">
        ...
        <div className="flex justify-between items-start">
          <div>
            <span className="text-[10px] bg-brand-light text-brand dark:bg-brand/10 dark:text-brand-light px-2 py-0.5 rounded font-bold uppercase tracking-wider">의뢰인 스텔스 보안</span>
        ...
        <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-xl">
          <button type="button" onClick={() => setAuthTab('phone')} className={`flex-1 text-center py-2 text-xs font-bold rounded-lg transition-all ${authTab === 'phone' ? 'bg-white dark:bg-slate-800 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-850 dark:hover:text-slate-350'}`}>휴대폰/SNS 로그인</button>
          <button type="button" onClick={() => setAuthTab('email')} className={`flex-1 text-center py-2 text-xs font-bold rounded-lg transition-all ${authTab === 'email' ? 'bg-white dark:bg-slate-800 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-850 dark:hover:text-slate-350'}`}>이메일 로그인</button>
        </div>

        {authTab === 'phone' ? (
          <div className="space-y-4 animate-fadeIn">
            <div className="space-y-2.5">
              <button onClick={() => handleSocialLogin('Google')} className="w-full bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-sm text-sm">
              ...
              <button onClick={() => handleSocialLogin('카카오')} className="w-full bg-[#FEE500] hover:bg-[#FEE500]/95 text-[#191919] font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-sm text-sm">
                <span className="w-4 h-4 flex items-center justify-center font-extrabold text-xs bg-[#3c2a2b] text-[#FEE500] rounded-full">K</span><span>카카오로 간편 로그인</span>
              </button>
              <button onClick={() => handleSocialLogin('네이버')} className="w-full bg-[#03C75A] hover:bg-[#03C75A]/95 text-white font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-sm text-sm">
                <span className="w-4 h-4 flex items-center justify-center font-black text-xs bg-white text-[#03C75A] rounded-full">N</span><span>네이버로 간편 로그인</span>
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="tel" placeholder="010-1234-5678" value={authPhone} onChange={(e) => setAuthPhone(e.target.value)} disabled={otpSent} className="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-750 rounded-xl p-3 text-sm focus:ring-1 focus:ring-brand disabled:opacity-50" />
              ...
              {otpSent && (
                <div className="space-y-2 animate-slideDown">
                  ...
                  <div className="flex gap-2">
                    <input type="text" placeholder="인증번호 6자리 입력 (777777)" value={authOtp} onChange={(e) => setAuthOtp(e.target.value)} maxLength={6} className="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-750 rounded-xl p-3 text-sm focus:ring-1 focus:ring-brand" />
                    <button onClick={handleVerifyOtp} className="bg-brand hover:bg-brand text-white font-bold px-4 py-3 rounded-xl text-xs transition-colors shrink-0">인증 및 로그인</button>
                  </div>
                </div>
              )}
          ...
          <form onSubmit={handleEmailAuth} className="space-y-3.5 animate-fadeIn">
            <div>
              ...
              <input type="email" placeholder="example@email.com" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} required className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-750 rounded-xl p-3 text-sm focus:ring-1 focus:ring-brand focus:outline-none" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-300 mb-1">비밀번호</label>
              <input type="password" placeholder="비밀번호 입력 (6자리 이상)" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} required minLength={6} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-750 rounded-xl p-3 text-sm focus:ring-1 focus:ring-brand focus:outline-none" />
            </div>
            {isRegisterMode && (
              <div className="animate-slideDown">
                <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-300 mb-1">의뢰인 가명 지정 (생략 시 자동 부여)</label>
                <input type="text" placeholder="예: 새출발_777" value={userAlias} onChange={(e) => setUserAlias(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-750 rounded-xl p-3 text-sm focus:ring-1 focus:ring-brand focus:outline-none" />
              </div>
            )}
            <button type="submit" className="w-full text-center py-3 bg-brand hover:bg-brand-hover text-white text-sm font-bold rounded-xl transition-all shadow-sm flex items-center justify-center gap-1.5 cursor-pointer">
              <Lock className="w-3.5 h-3.5" /><span>{isRegisterMode ? '스텔스 회원가입 및 로그인' : '이메일 안전 로그인'}</span>
            </button>
            <div className="text-center pt-1">
              <button type="button" onClick={() => setIsRegisterMode(!isRegisterMode)} className="text-[11px] text-slate-450 hover:text-brand transition-colors font-semibold underline cursor-pointer">
        ...
        <div className="flex items-start gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          <input type="checkbox" id="authConsent" checked={authConsent} onChange={(e) => setAuthConsent(e.target.checked)} className="mt-0.5 rounded border-slate-300 dark:border-slate-750 text-brand focus:ring-brand w-4 h-4 shrink-0" />
