    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white dark:bg-slate-900 border-t sm:border border-slate-200 dark:border-slate-800 rounded-t-3xl sm:rounded-3xl max-w-full sm:max-w-md w-full shadow-2xl p-6 md:p-8 space-y-4 relative overflow-hidden text-left animate-slideUp sm:animate-fadeIn max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start">
          <div>
            <span className="text-[12px] bg-brand-light text-brand dark:bg-brand/10 dark:text-brand-light px-2 py-0.5 rounded font-semibold uppercase tracking-wider">의뢰인 스텔스 보안</span>
            <h3 className="font-bold text-xl text-slate-900 dark:text-white mt-1">로그인 및 회원가입</h3>
            <p className="text-xs text-slate-600 mt-1">채무 사실 노출을 막기 위해 가명 닉네임과 가명 발신자 알림 시스템이 자동으로 활성화됩니다.</p>
          </div>
        ...
        <div className="space-y-2.5">
          <button onClick={() => handleSocialLogin('Google')} className="w-full bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-900 dark:text-slate-200 border border-slate-200 dark:border-slate-700 font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-sm text-sm cursor-pointer">
        ...
        <div className="relative flex py-1 items-center">
          ...
          <span className="flex-shrink mx-4 text-slate-500 text-[12px] font-semibold">또는 이메일로 안전 로그인</span>
          <div className="flex-grow border-t border-slate-200 dark:border-slate-800"></div>
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-3.5">
          <div>
            <label className="block text-[12px] font-semibold text-slate-700 dark:text-slate-300 mb-1">이메일 주소</label>
          ...
          <div>
            <label className="block text-[12px] font-semibold text-slate-700 dark:text-slate-300 mb-1">비밀번호</label>
          ...
          {isRegisterMode && (
            <div className="animate-slideDown">
              <label className="block text-[12px] font-semibold text-slate-700 dark:text-slate-300 mb-1">의뢰인 가명 지정 (생략 시 자동 부여)</label>
          ...
          <div className="text-center pt-1">
            <button type="button" onClick={() => setIsRegisterMode(!isRegisterMode)} className="text-[13px] text-slate-500 hover:text-brand transition-colors font-semibold underline cursor-pointer">
        ...
        <div className="flex items-start gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          <label htmlFor="authConsent" className="text-[13px] text-slate-600 dark:text-slate-400 select-none cursor-pointer leading-normal">
        ...
        <div className="text-center pt-1">
          <button
            ...
            className="text-[13px] text-slate-500 hover:text-indigo-500 transition-colors font-semibold cursor-pointer inline-flex items-center gap-1"
          >
