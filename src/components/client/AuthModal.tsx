      ...
      <div className="bg-white dark:bg-slate-900 border-t sm:border border-slate-200 dark:border-slate-800 rounded-t-3xl sm:rounded-3xl max-w-full sm:max-w-md w-full shadow-2xl p-6 md:p-8 space-y-4 relative text-left animate-slideUp sm:animate-fadeIn max-h-[85dvh] sm:max-h-[90vh] overflow-y-auto overscroll-contain">
        ...
        <div className="space-y-2.5">
          <div className="grid grid-cols-2 gap-2.5 pt-1">
            <button onClick={() => handleSocialLogin('Google')} className="bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-900 dark:text-slate-200 border border-slate-200 dark:border-slate-700 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs text-sm cursor-pointer">
              <span className="w-5 h-5 flex items-center justify-center font-extrabold text-xs bg-red-500 text-white rounded-full">G</span><span>Google 로그인</span>
            </button>
            <button onClick={() => handleSocialLogin('카카오')} className="bg-[#FEE500] hover:bg-[#FEE500]/95 text-[#191919] font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs text-sm cursor-pointer">
              <span className="w-5 h-5 flex items-center justify-center font-extrabold text-xs bg-[#3c2a2b] text-[#FEE500] rounded-full">K</span><span>카카오 로그인</span>
            </button>
          </div>
        </div>
        ...
        <div className="relative flex py-1.5 items-center">
          ...
          <span className="flex-shrink mx-4 text-slate-500 text-xs sm:text-sm font-semibold">또는 이메일 직접 계정 사용</span>
          <div className="flex-grow border-t border-slate-200 dark:border-slate-800"></div>
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-3.5">
          <div>
            <label className="block text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">이메일 주소</label>
            <input type="email" placeholder="example@email.com" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} required className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-base focus:ring-1 focus:ring-brand focus:outline-none font-medium" />
          </div>
          <div>
            <label className="block text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">비밀번호</label>
            <input type="password" placeholder="비밀번호 입력 (6자리 이상)" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} required minLength={6} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-base focus:ring-1 focus:ring-brand focus:outline-none font-medium" />
          </div>
          {isRegisterMode && (
          ...
          <button type="submit" className="w-full text-center py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-sm sm:text-base font-bold rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer">
            <Lock className="w-4 h-4 text-brand" /><span>{isRegisterMode ? '이메일 회원가입' : '이메일 로그인'}</span>
          </button>
          <div className="text-center pt-1">
            <button type="button" onClick={() => setIsRegisterMode(!isRegisterMode)} className="text-xs sm:text-sm text-slate-500 hover:text-brand transition-colors font-bold underline cursor-pointer">
        ...
        <div className="flex items-start gap-2.5 pt-3 border-t border-slate-100 dark:border-slate-800">
          <input type="checkbox" id="authConsent" checked={authConsent} onChange={(e) => setAuthConsent(e.target.checked)} className="mt-1 rounded border-slate-300 dark:border-slate-700 text-brand focus:ring-brand w-4 h-4 shrink-0" />
          <label htmlFor="authConsent" className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 select-none cursor-pointer leading-normal">
        ...
        <div className="text-center pt-1 pb-2">
          <button
            ...
            className="text-sm text-slate-600 hover:text-indigo-600 transition-colors font-bold cursor-pointer inline-flex items-center gap-1.5"
          >
            <span className="w-4 h-4 flex items-center justify-center bg-indigo-100 dark:bg-indigo-900/30 text-indigo-500 rounded-full text-xs font-bold">⚖</span>
