    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 flex items-center justify-around py-3 pb-safe-bottom shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
      ...
      <button
        ...
        className={`flex flex-col items-center gap-1 flex-1 text-center transition-colors relative ${
          activeTab === 'landing' ? 'text-brand font-bold' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
        }`}
      >
        {activeTab === 'landing' && <span className="absolute -top-1 left-1/2 -translate-x-1/2 w-10 h-10 bg-brand/10 rounded-full" />}
        <Home className="w-5 h-5 relative z-10" />
        <span className="text-[10px] tracking-tight relative z-10">홈</span>
      </button>

      {/* 채무관리 시작 */}
      <button
        onClick={onStartDiagnosis}
        className="flex flex-col items-center gap-1 flex-1 text-center transition-colors relative text-slate-400 dark:text-slate-500 hover:text-slate-600"
      >
      ...
      <button
        ...
        className={`flex flex-col items-center gap-1 flex-1 text-center transition-colors relative ${
          activeTab === 'request' ? 'text-brand font-bold' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
        }`}
      >
        ...
        <span className="text-[10px] tracking-tight relative z-10">무료 전담배정</span>
      </button>
      ...
      <button
        ...
        className={`flex flex-col items-center gap-1 flex-1 text-center transition-colors relative ${
          activeTab === 'chat' ? 'text-brand font-bold' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'
        }`}
      >
        {activeTab === 'chat' && <span className="absolute -top-1 left-1/2 -translate-x-1/2 w-10 h-10 bg-brand/10 rounded-full" />}
        <MessageSquare className="w-5 h-5 relative z-10" />
        <span className="text-[10px] tracking-tight relative z-10">내 관리방</span>
