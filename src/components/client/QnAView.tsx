    ...
    <div className="space-y-8 animate-fadeIn text-left">
      ...
      {filteredQAs.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-12 rounded-2xl text-center space-y-3">
          ...
          <h4 className="font-semibold text-sm text-slate-800 dark:text-slate-200">일치하는 상담사례가 없습니다.</h4>
                  ...
                  <div onClick={() => setOpenedQaId(isOpen ? null : qa.id)} className="p-5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 flex items-start justify-between gap-4">
                    <div className="space-y-2 text-left">
                      <div className="flex items-center gap-2.5">
                        <span className="bg-brand-light text-brand dark:bg-brand/10 dark:text-indigo-400 text-[10px] font-bold px-2.5 py-0.5 rounded-md">{qa.category}</span>
                        <span className="text-[10px] text-slate-400 font-semibold">{qa.author}</span>
                        <div className="flex items-center gap-1.5 ml-auto">
                          ...
                          <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">{qa.lawyerName} 답변</span>
                        </div>
                      </div>
                      <h4 className="font-semibold text-sm sm:text-base text-slate-800 dark:text-slate-200 pr-4 leading-snug">Q. {qa.question}</h4>
                    </div>
                    ...
                    <div className="px-5 pb-5 border-t border-slate-100 dark:border-slate-800 pt-4 bg-slate-50/50 dark:bg-slate-950/20 text-left space-y-4 animate-slideDown">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold px-2 py-0.5 rounded-md">전문가 답변</span>
                          </div>
                      ...
                      <div className="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
                        <button
                          ...
                          className="bg-gradient-to-r from-brand to-indigo-600 hover:from-brand-hover hover:to-indigo-700 text-white font-bold px-4 py-2 rounded-2xl text-[10px] transition-all shadow-sm hover:shadow-brand-sm active:scale-[0.98] cursor-pointer"
                        >
