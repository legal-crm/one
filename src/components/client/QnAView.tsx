    ...
    <div className="space-y-8 animate-fadeIn text-left">
      ...
      <div className="bg-[#0F172A] border border-slate-800 rounded-3xl p-6 md:p-10 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="max-w-2xl relative z-10 space-y-4">
          ...
          <h1 className="text-2xl md:text-3.5xl font-black tracking-tight leading-tight">회생톡 법률 상담사례 디렉토리</h1>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap gap-1.5 pt-3 border-t border-slate-150/40 dark:border-slate-800">
          {QNA_CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => { setCategoryFilter(cat); setPage(1); }} className={`px-3 py-1.5 text-[10px] sm:text-xs font-bold rounded-xl transition-all border cursor-pointer ${categoryFilter === cat ? 'bg-brand border-brand text-white shadow-sm' : 'bg-slate-50 hover:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-850 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400'}`}>
              {cat}
            </button>
          ))}
        </div>
                  ...
                  <div onClick={() => setOpenedQaId(isOpen ? null : qa.id)} className="p-5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 flex items-start justify-between gap-4">
                    <div className="space-y-2 text-left">
                      <div className="flex items-center gap-2.5">
                        <span className="bg-brand-light text-brand dark:bg-brand/10 dark:text-blue-450 text-[9px] font-extrabold px-2.5 py-0.5 rounded-md">{qa.category}</span>
                      ...
                      <h4 className="font-extrabold text-sm sm:text-base text-slate-850 dark:text-slate-200 pr-4 leading-snug">Q. {qa.question}</h4>
                    </div>
