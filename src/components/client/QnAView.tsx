...
export default function QnAView({ qas, onConsultRequest }: QnAViewProps) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn text-left">
      {/* Page Header */}
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-12 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="max-w-2xl relative z-10 space-y-4">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight">my김변 법률 상담사례 디렉토리</h1>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-sm font-bold text-slate-500 block uppercase">실시간 키워드 검색</span>
            <div className="relative w-full md:w-80">
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm focus:ring-1 focus:ring-brand focus:outline-none"
              />
            </div>
          </div>
          <div className="text-right">
            <span className="text-sm font-bold text-slate-500 block uppercase mb-1">총 매칭 사례</span>
      ...
      {filteredQAs.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-12 rounded-2xl text-center space-y-3">
          ...
          <button onClick={() => { setCategoryFilter('전체'); setSearchQuery(''); setPage(1); }} className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-sm font-bold rounded-xl transition-colors cursor-pointer">
            필터 초기화
          </button>
        </div>
      ) : (
                  ...
                  <div onClick={() => setOpenedQaId(isOpen ? null : qa.id)} className="p-5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 flex items-start justify-between gap-4">
                    <div className="space-y-2 text-left">
                      <div className="flex items-center gap-2.5">
                        <span className="bg-brand-light text-brand dark:bg-brand/10 dark:text-indigo-400 text-xs font-bold px-2.5 py-0.5 rounded-md">{qa.category}</span>
                        <span className="text-xs text-slate-500 font-semibold">{qa.author}</span>
                        <div className="flex items-center gap-1.5 ml-auto">
                          ...
                          <span className="text-xs font-bold text-slate-600 dark:text-slate-400">{qa.lawyerName} 답변</span>
                        </div>
                      </div>
                      <h4 className="font-semibold text-base sm:text-lg text-slate-900 dark:text-slate-200 pr-4 leading-snug">Q. {qa.question}</h4>
                    </div>
                    ...
                    <div className="px-5 pb-5 border-t border-slate-100 dark:border-slate-800 pt-4 bg-slate-50/50 dark:bg-slate-950/20 text-left space-y-4 animate-slideDown">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold px-2 py-0.5 rounded-md">전문가 답변</span>
                          </div>
                          <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-normal pt-1.5 whitespace-pre-wrap text-left">{qa.answer}</p>
                        </div>
                      </div>
                      <div className="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
                        <button
                          ...
                          className="bg-gradient-to-r from-brand to-indigo-600 hover:from-brand-hover hover:to-indigo-700 text-white font-bold px-5 py-2.5 rounded-2xl text-xs transition-all shadow-sm hover:shadow-brand-sm active:scale-[0.98] cursor-pointer"
                        >
