    ...
    <div className="w-full">
      ...
      <section className="w-full bg-gradient-to-br from-[#0F2440] via-[#1E3A5F] to-[#162D4A] py-20 md:py-28 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center space-y-5">
          ...
          <p className="text-base md:text-lg text-slate-200 font-medium max-w-xl mx-auto leading-relaxed">
      ...
      <section className="w-full py-16 md:py-24 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          <p className="text-sm font-bold text-[#3B82F6] tracking-widest uppercase">Mission</p>
          <h2 className="text-2xl md:text-3xl font-extrabold text-[#0f172a] leading-snug tracking-tight">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {[
              ...
              <div key={idx} className="text-center space-y-2">
                ...
                <p className="text-sm md:text-base text-slate-600 font-semibold">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="w-full py-16 md:py-24 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
          <div className="text-center space-y-2">
            ...
            <p className="text-base text-slate-500 font-medium">my김변이 지키는 3가지 원칙</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              },
              {
                ...
                title: '접근성',
                desc: '복잡한 법률 절차를 1분 채무 체크와 다수 변호사 선택 상담으로 간소화하여 누구나 쉽게 시작할 수 있습니다.',
              },
            ].map((item, idx) => (
              <div key={idx} className="text-center space-y-3.5 p-6 bg-slate-50/60 rounded-2xl border border-slate-100">
                <div className="w-14 h-14 mx-auto rounded-2xl bg-[#EEF4FA] flex items-center justify-center text-[#1E3A5F]">
                  {item.icon}
                </div>
                <h4 className="font-bold text-lg text-[#0f172a]">{item.title}</h4>
                <p className="text-base text-slate-600 leading-relaxed font-normal">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="w-full py-16 md:py-24 bg-slate-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { title: '변호사법 준수', desc: '변호사법 제34조에 의거 알선료·수수료 수취를 금지하는 구조를 철저히 채택하고 있습니다.' },
              { title: '광고비 정액제', desc: '광고비는 정액제로 상담 건수·수임 여부·사건 결과와 무관한 고정금액입니다.' },
              { title: '통신판매중개자', desc: '플랫폼은 통신판매중개자로서 변호사와 의뢰인 간 계약에 직접 관여하지 않습니다.' },
              { title: '개인정보 보호', desc: '개인정보 처리방침에 따라 이용자의 정보를 안전하게 암호화하여 관리합니다.' },
            ].map((item, idx) => (
              <div key={idx} className="bg-white rounded-2xl border border-slate-200 p-6 space-y-2 text-left">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-[#0F766E]" />
                  <h5 className="font-bold text-base text-[#0f172a]">{item.title}</h5>
                </div>
