...
export default function ReviewsView({ reviews, onReviewClick }: ReviewsViewProps) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn text-left font-sans">
      {/* Page Header */}
      <div className="bg-gradient-to-br from-indigo-950 via-slate-900 to-indigo-950 rounded-[32px] p-6 md:p-10 text-white shadow-2xl relative overflow-hidden border border-slate-800/80 transition-all duration-300">
        ...
        <div className="max-w-2xl relative z-10 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-5 pt-4 border-t border-slate-700/50">
            <div className="space-y-1">
              ...
              <span className="block text-sm md:text-lg font-black text-amber-400">84억 5,000만원+</span>
            </div>
            <div className="space-y-1">
              <span className="block text-[10px] md:text-xs text-slate-400 font-bold">인가면책 성공률</span>
              <span className="block text-sm md:text-lg font-black text-indigo-400">98.7%</span>
            </div>
            <div className="space-y-1">
              <span className="block text-[10px] md:text-xs text-slate-400 font-bold">평균 감면율</span>
              <span className="block text-sm md:text-lg font-black text-emerald-400">최대 78%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-100/80 dark:border-slate-800/85 p-6 rounded-2xl shadow-premium space-y-4">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <span className="text-xs text-slate-450 dark:text-slate-400 font-extrabold self-end md:self-center">
            ...
            <button
              ...
              className={`px-4 py-2 text-xs font-black rounded-xl transition-all duration-300 border cursor-pointer ${
                categoryFilter === cat
                  ...
                  : 'bg-slate-50/50 hover:bg-slate-100 dark:bg-slate-950/40 dark:hover:bg-slate-850/85 border-slate-200/50 dark:border-slate-800/80 text-slate-600 dark:text-slate-400'
              }`}
            >
              {cat}
            </button>
      ...
      {filteredReviews.length === 0 ? (
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 p-12 rounded-2xl text-center space-y-3 shadow-premium">
          ...
          <h4 className="font-extrabold text-sm text-slate-800 dark:text-slate-200">일치하는 성공 사례가 없습니다.</h4>
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedReviews.map(rev => {
              ...
              return (
                <div key={rev.id} className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border border-slate-100/80 dark:border-slate-800/85 rounded-3xl shadow-premium hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 p-6 flex flex-col justify-between space-y-6 group relative overflow-hidden text-left">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="bg-brand/5 border border-brand/20 text-brand dark:text-brand-light text-[10px] font-black px-3 py-1 rounded-full shadow-sm">{rev.category}</span>
                      <div className="flex text-amber-400 text-xs tracking-wider">★★★★★</div>
                    </div>
                    <h3 className="font-extrabold text-sm sm:text-base text-slate-850 dark:text-white leading-snug group-hover:text-brand dark:group-hover:text-brand-light transition-colors line-clamp-2">
                      "{rev.title}"
                    </h3>
                    <div className="bg-slate-50/60 dark:bg-slate-950/40 p-4 rounded-2xl border border-slate-150/40 dark:border-slate-850/80 space-y-2.5 text-xs font-bold shadow-sm">
                      ...
                      <div className="flex justify-between items-center text-slate-850 dark:text-white font-extrabold">
                        <span>조정 후 채무</span>
                      ...
                      <div className="flex justify-between items-center pt-2.5 border-t border-slate-200/50 dark:border-slate-800/50 text-[10px] font-bold">
                        <span className="text-emerald-600 dark:text-emerald-450">총 감면 혜택</span>
                        <span className="bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-450 px-2 py-0.5 rounded font-black">
                          {reductionRate}% 감면 (-{saved.toLocaleString()}만원)
                        </span>
                      </div>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-semibold line-clamp-3 md:line-clamp-none">{rev.content}</p>
                    <div className="flex flex-wrap gap-1.5 pt-1.5">
                      {rev.tags.map(t => (
                        <button key={t} onClick={() => { setSearchQuery(t); setPage(1); }} className="text-[10px] text-slate-500 dark:text-slate-450 bg-slate-50 hover:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-850 border border-slate-150/30 dark:border-slate-800/80 px-2 py-1 rounded-md hover:text-brand-hover dark:hover:text-brand-light transition-colors font-extrabold cursor-pointer">
                          #{t}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800/80">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-slate-455 font-bold">{rev.author} 의뢰인님</span>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                          <span className="block font-black text-slate-700 dark:text-slate-300 text-[11px] leading-none">{rev.lawyerName}</span>
                          <span className="text-[9px] text-slate-400 font-bold">도산 전담 변호사</span>
                        </div>
                      </div>
                    </div>
                    <button onClick={() => onReviewClick(rev)} className="w-full text-center py-3 bg-gradient-to-r from-brand to-indigo-600 hover:from-brand-hover hover:to-indigo-700 text-white text-xs font-black rounded-2xl transition-all duration-300 flex items-center justify-center gap-1.5 shadow-sm hover:shadow-[0_4px_10px_rgba(114,100,255,0.2)] cursor-pointer transform hover:-translate-y-0.5">
