...
export default function ReviewsView({ reviews, onReviewClick }: ReviewsViewProps) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn text-left font-sans">
      {/* Page Header */}
      <div className="bg-gradient-to-br from-indigo-950 via-slate-900 to-indigo-950 rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden border border-slate-800 transition-all duration-300">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:max-w-md">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-10 py-3 text-sm focus:ring-1 focus:ring-brand focus:outline-none font-bold"
            />
            {searchQuery && (
          ...
          <span className="text-sm text-slate-500 dark:text-slate-400 font-bold self-end md:self-center">
        ...
        <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-4 py-2 text-sm font-bold rounded-xl transition-all duration-300 border cursor-pointer ${
                categoryFilter === cat
      ...
      {filteredReviews.length === 0 ? (
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 p-12 rounded-2xl text-center space-y-3 shadow-premium">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-sm font-bold rounded-xl transition-colors cursor-pointer"
          >
            필터 초기화
          </button>
        </div>
              ...
              return (
                <div key={rev.id} className="bg-slate-50/90 dark:bg-slate-850/50 backdrop-blur-md border border-slate-200/60 dark:border-slate-800 rounded-3xl shadow-premium hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 p-6 flex flex-col justify-between space-y-6 group relative overflow-hidden text-left">
                  ...
                  <div className="space-y-4">
                    <h3 className="font-semibold text-base sm:text-lg text-slate-900 dark:text-white leading-snug group-hover:text-brand dark:group-hover:text-brand-light transition-colors line-clamp-2">
                      "{rev.title}"
                    </h3>
                    <p className="text-base text-slate-600 dark:text-slate-400 leading-relaxed font-medium line-clamp-3 md:line-clamp-none">{rev.content}</p>
                    <div className="flex flex-wrap gap-1.5 pt-1.5">
                      {rev.tags.map(t => (
                        <button key={t} onClick={() => { setSearchQuery(t); setPage(1); }} className="text-xs text-slate-600 dark:text-slate-400 bg-slate-50 hover:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 px-2 py-1 rounded-md hover:text-brand-hover dark:hover:text-brand-light transition-colors font-bold cursor-pointer">
                          #{t}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                          ...
                          <span className="text-sm text-slate-500 font-medium">도산 전담 변호사</span>
                        </div>
                      </div>
                    </div>
                    <button onClick={() => onReviewClick(rev)} className="w-full text-center py-3 bg-gradient-to-r from-brand to-indigo-600 hover:from-brand-hover hover:to-indigo-700 text-white text-sm font-bold rounded-2xl transition-all duration-300 flex items-center justify-center gap-1.5 shadow-sm hover:shadow-brand-sm cursor-pointer transform hover:-translate-y-0.5 active:scale-[0.98]">
