  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors active:scale-[0.98] cursor-pointer ${
              filter === 'all' ? 'bg-brand text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
            }`}
          >
            \uC804\uCCB4
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors active:scale-[0.98] cursor-pointer flex items-center gap-1 ${
              filter === 'message' ? 'bg-brand text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
            }`}
          >
            \uD83D\uDCAC \uB300\uD654
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors active:scale-[0.98] cursor-pointer flex items-center gap-1 ${
              filter === 'task' ? 'bg-brand text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
            }`}
          >
            \uD83D\uDCCB \uC5C5\uBB34
          </button>
      ...
      {pinnedMessages.length > 0 && (
        <div className="mb-4 bg-amber-50/50 rounded-xl p-2.5 border border-amber-100 flex flex-col gap-2">
          <div className="flex items-center gap-1.5 text-amber-700 text-xs font-semibold px-1">
            <Pin className="w-3.5 h-3.5" />
            \uACE0\uC815\uB41C \uBA54\uC2DC\uC9C0
          </div>
          {pinnedMessages.map(msg => (
            <div key={msg.id} className="bg-white/80 rounded-lg p-2 text-sm text-slate-700 flex justify-between items-start shadow-sm">
      ...
      {/* Timeline */}
      <div className="flex-1 overflow-y-auto max-h-[400px] flex flex-col gap-3 pr-1 pb-4">
          ...
          filteredItems.map(item => (
            <div key={item.id} className="flex flex-col">
              {item.type === 'message' ? (
                <div className="bg-slate-50 rounded-xl p-3 flex gap-3 group relative">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-sm text-slate-800">{item.authorName}</span>
                  ...
                  <button 
                    ...
                    className={`absolute top-3 right-3 p-1 rounded-md transition-colors cursor-pointer ${
                      item.isPinned ? 'text-amber-500' : 'text-slate-300 hover:text-slate-500 hover:bg-slate-200 opacity-0 group-hover:opacity-100'
                    }`}
                  >
                ...
                <div className={`bg-white shadow-sm rounded-xl p-3.5 border border-slate-100 border-l-4 ${getStatusBorder(item.taskStatus)}`}>
                    ...
                    <div className="flex items-center gap-2">
                      {item.taskStatus === 'pending' && (
                        <button 
                          ...
                          className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium rounded-lg transition-colors active:scale-[0.98] cursor-pointer"
                        >
                          \uC2DC\uC791
                        </button>
                      )}
                      ...
                      {item.taskStatus === 'in_progress' && completingTaskId !== item.id && (
                        <button 
                          onClick={() => setCompletingTaskId(item.id)}
                          className="px-2.5 py-1 bg-brand text-white text-xs font-medium rounded-lg transition-colors active:scale-[0.98] cursor-pointer"
                        >
                          \uC644\uB8CC
                        </button>
                      )}
                  ...
                  {completingTaskId === item.id && (
                    <div className="mt-3 flex gap-2 animate-in fade-in slide-in-from-top-2">
                      <input 
                        ...
                        className="flex-1 text-sm border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand"
                        autoFocus
                      />
                      <button 
                        onClick={() => {
                          updateTaskStatus(item.id, 'completed', completionNote);
                          setCompletingTaskId(null);
                          setCompletionNote('');
                        }}
                        className="px-3 py-1.5 bg-brand text-white text-xs font-medium rounded-lg transition-colors active:scale-[0.98] cursor-pointer"
                      >
                        \uD655\uC778
                      </button>
                      <button 
                        ...
                        className="px-3 py-1.5 bg-slate-100 text-slate-600 text-xs font-medium rounded-lg transition-colors active:scale-[0.98] cursor-pointer"
                      >
                        \uCDE8\uC18C
                      </button>
                    </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-slate-100 pt-3 mt-auto">
        {isTaskMode && (
          <div className="flex flex-wrap gap-2 mb-2 p-2 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div className="flex flex-col gap-1 w-32">
              <label className="text-xs font-medium text-slate-500">\uC6B0\uC120\uC21C\uC704</label>
              <select 
                value={taskPriority} 
            ...
            <div className="flex flex-col gap-1 w-32">
              <label className="text-xs font-medium text-slate-500">\uAE30\uD55C</label>
              <input 
                type="date" 
                value={taskDueDate}
        ...
        <div className="flex items-end gap-2">
          <button
            ...
            className={`p-2.5 rounded-xl transition-colors active:scale-[0.98] cursor-pointer ${
              isTaskMode 
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand resize-none bg-slate-50 focus:bg-white transition-colors"
              rows={1}
          ...
          <button
            ...
            className="p-2.5 bg-brand text-white rounded-xl hover:bg-brand/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] cursor-pointer shrink-0"
          >
