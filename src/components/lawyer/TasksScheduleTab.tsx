          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex gap-1.5 flex-wrap">
              ...
              {(['all' as const, ...(['firm', 'lawyers', 'personal'] as EventVisibility[])]).map(v => {
                ...
                if (v === 'all') return <button key={v} onClick={() => setVisFilter('all')} className={`px-2.5 py-1 text-xs font-bold rounded-lg border cursor-pointer active:scale-[0.98] transition-all ${active ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>{'\uC804\uCCB4'}</button>;
                const vc = VISIBILITY_CONFIG[v];
                return <button key={v} onClick={() => setVisFilter(visFilter === v ? 'all' : v)} className={`px-2.5 py-1 text-xs font-bold rounded-lg border cursor-pointer active:scale-[0.98] transition-all flex items-center gap-1 ${active ? vc.bgColor + ' ' + vc.color + ' border-current/20' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>{vc.emoji} {vc.label}</button>;
              })}
            </div>
