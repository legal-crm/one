...
export default function TasksScheduleTab({ tenantId, userId, userName, userRole, hasManageCalendar, requests, cases, qas, activeLawyerId }: TasksScheduleTabProps) {
  ...
  const [typeFilters, setTypeFilters] = useState<Record<EventType, boolean>>({ court: true, consult: true, meeting: true, deadline: true, other: true });
  ...
  const handleDeleteEvent = async (evt: CalendarEvent) => {
    if (!canDeleteEvent(evt, userId, userRole, hasManageCalendar)) { toast.error('\uC0AD\uC81C \uAD8C\uD55C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4'); return; }
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
                  ...
                  <div key={task.id} className={`bg-white rounded-2xl border shadow-sm p-4 ${overdue ? 'border-red-200 bg-red-50/30' : 'border-slate-200'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1 text-[11px] text-slate-400"><span>{task.assignerName}</span>{task.dueDate && <span className="flex items-center gap-0.5"><Calendar className="w-3 h-3" />{task.dueDate}</span>}</div>
                      </div>
                      {task.status !== 'COMPLETED' && (
                        <div className="flex flex-col gap-1.5 shrink-0">
                          {task.status === 'PENDING' && <button onClick={() => handleStart(task.id)} className="bg-blue-50 text-blue-600 rounded-xl px-3 py-1.5 text-[11px] font-bold hover:bg-blue-100 active:scale-[0.98] transition-all whitespace-nowrap cursor-pointer">{'\uC2DC\uC791'}</button>}
        ...
        <div className="space-y-4">
            ...
            <div className="bg-brand/5 border border-brand/15 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 space-y-0.5">
                  {todayEvents.map(e => { const cfg = EVENT_TYPE_CONFIG[e.type]; return <p key={e.id} className="text-xs text-slate-600"><span className="font-bold">{cfg.emoji}</span> {e.startTime && <span className="text-slate-400">{e.startTime} </span>}{e.title} {e.visibility !== 'personal' && <span className="text-[10px] text-slate-400">{visEmoji(e.visibility)}</span>}</p>; })}
                  {todayTasks.map(t => <p key={t.id} className="text-xs text-slate-600">{'\uD83D\uDCCB'} {t.title}</p>)}
                </div>
              </div>
            </div>
          )}
          ...
          {upcomingCourt.length > 0 && (
            <div className="bg-red-50/60 border border-red-200 rounded-2xl p-4">
              ...
              <div className="flex gap-3 flex-wrap">
                {upcomingCourt.map(e => { const dd = dDay(e.date); return (
                    ...
                    {(Object.keys(EVENT_TYPE_CONFIG) as EventType[]).map(type => {
                      ...
                      return (
                        <button key={type} onClick={() => setTypeFilters(p => ({ ...p, [type]: !p[type] }))}
                          className={`px-2 py-1 text-[11px] font-bold rounded-lg border transition-all cursor-pointer active:scale-[0.98] flex items-center gap-1 ${typeFilters[type] ? cfg.bgColor + ' ' + cfg.color + ' border-current/20' : 'bg-slate-50 text-slate-300 border-slate-100 line-through'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex bg-slate-100 rounded-lg p-0.5 gap-0.5">
                    {(['all' as const, ...(['firm', 'lawyers', 'personal'] as EventVisibility[])]).map(v => {
                      ...
                      if (v === 'all') return <button key={v} onClick={() => setVisFilter('all')} className={`px-2.5 py-1.5 text-[11px] font-bold rounded-md cursor-pointer transition-all ${active ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{'\uC804\uCCB4'}</button>;
                      const vc = VISIBILITY_CONFIG[v];
                      return <button key={v} onClick={() => setVisFilter(visFilter === v ? 'all' : v)} className={`px-2.5 py-1.5 text-[11px] font-bold rounded-md cursor-pointer transition-all flex items-center gap-1 ${active ? 'bg-white shadow-sm ' + vc.color : 'text-slate-400 hover:text-slate-600'}`}>{vc.emoji} {vc.label}</button>;
                    })}
                  </div>
                </div>
              </div>

              <button onClick={() => { const dk = selectedDay ? toDateKey(calYear, calMon, selectedDay) : todayKey; openAddModalForDate(dk); }}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {calView === 'month' ? (<>
                    ...
                    <h3 className="text-lg font-black text-white min-w-[200px] text-center">{(weekStart.getMonth() + 1) + '\uC6D4 ' + weekStart.getDate() + '\uC77C ~ ' + (weekDays[6].getMonth() + 1) + '\uC6D4 ' + weekDays[6].getDate() + '\uC77C'}</h3>
            ...
            <div className="grid grid-cols-7 border-b border-slate-200">
              {DAY_HEADERS.map((d, i) => <div key={d} className={`text-center text-sm font-bold py-3 ${i === 0 ? 'text-red-400' : i === 6 ? 'text-blue-400' : 'text-slate-500'} bg-slate-50`}>{d}</div>)}
            </div>
            ...
            {calView === 'month' && (
              <div className="grid grid-cols-7">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  const allItems = [...dayEvents.map(e => ({ k: 'e' as const, e })), ...dayTasks.map(t => ({ k: 't' as const, t }))];
                  return (
                    <button key={day} onClick={() => setSelectedDay(sel ? null : day)}
                      ...
                      <div className="flex items-start justify-between mb-1">
                        {isT ? <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-brand text-white text-sm font-black">{day}</span>
                        ...
                        <div className="space-y-0.5">
                          {allItems.slice(0, 2).map((item, idx) => {
                            if (item.k === 'e') {
                              ...
                              return <div key={idx} className={`text-[10px] font-bold px-1.5 py-0.5 rounded truncate ${cfg.bgColor} ${cfg.color}`}>{item.e.visibility === 'firm' ? '\uD83C\uDFE2 ' : ''}{cfg.emoji} {item.e.title}</div>;
                            }
                            const pri = TASK_PRIORITY_CONFIG[(item.t as any).priority];
                            return <div key={idx} className={`text-[10px] font-bold px-1.5 py-0.5 rounded truncate ${pri.bgColor} ${pri.color}`}>{(item.t as any).title}</div>;
                          })}
                  ...
                  return (
                    <div key={i} className={`min-h-[200px] p-2 border-r border-slate-100 last:border-r-0 ${isT ? 'bg-brand/5' : ''}`}>
                      ...
                      <div className="space-y-1">
                        {dayEvents.map(e => { const cfg = EVENT_TYPE_CONFIG[e.type]; return <div key={e.id} className={`text-[10px] font-bold p-1.5 rounded-lg ${cfg.bgColor} ${cfg.color}`}><div className="truncate">{e.visibility === 'firm' ? '\uD83C\uDFE2 ' : ''}{cfg.emoji} {e.title}</div>{e.startTime && <div className="text-[9px] opacity-70">{e.startTime}</div>}</div>; })}
                        {dayTasks.map(t => { const pri = TASK_PRIORITY_CONFIG[t.priority]; return <div key={t.id} className={`text-[10px] font-bold p-1.5 rounded-lg ${pri.bgColor} ${pri.color}`}><div className="truncate">{t.title}</div></div>; })}
                ...
                {dayEvents.length === 0 && dayTasks.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {dayEvents.map(e => {
                      ...
                      const vc = VISIBILITY_CONFIG[e.visibility];
                      const canDel = canDeleteEvent(e, userId, userRole, hasManageCalendar);
                      return (
                        <div key={e.id} className={`flex items-center gap-3 p-3 rounded-xl ${cfg.bgColor}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[9px] font-bold px-1 py-0.5 rounded ${vc.bgColor} ${vc.color}`}>{vc.emoji} {vc.label}</span>
                            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(() => {
                ...
                return holidays.length > 0 ? holidays.map(h => <span key={h.day} className="text-xs bg-red-50 text-red-500 font-bold px-2 py-1 rounded-lg border border-red-100">{(calMon + 1) + '/' + h.day + ' ' + h.name}</span>) : <span className="text-xs text-slate-300">{'\uD734\uC77C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4'}</span>;
              })()}
            </div>
          </div>
        </div>
      ...
      {sub === 'activity' && (
        <div className="space-y-4">
          <div className="flex gap-2 flex-wrap">
            {([{ key: 'all', label: '\uC804\uCCB4' }, { key: 'request', label: '\uC0C1\uB2F4 \uC811\uC218' }, { key: 'counseling', label: '\uC0C1\uB2F4 \uC9C4\uD589' }, { key: 'case', label: '\uC218\uC784 \uC804\uD658' }, { key: 'qna', label: 'Q&A \uB2F5\uBCC0' }, { key: 'task', label: '\uD0DC\uC2A4\uD06C' }]).map(f => (
              <button key={f.key} onClick={() => setActivityFilter(f.key)} className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors cursor-pointer active:scale-[0.98] ${activityFilter === f.key ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{f.label}</button>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6 space-y-5 animate-fadeIn" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {(Object.keys(EVENT_TYPE_CONFIG) as EventType[]).map(type => {
                  const cfg = EVENT_TYPE_CONFIG[type];
                  return <button key={type} onClick={() => setNewEvt(p => ({ ...p, type }))} className={`px-3 py-1.5 text-xs font-bold rounded-xl border cursor-pointer active:scale-[0.98] transition-all flex items-center gap-1 ${newEvt.type === type ? cfg.bgColor + ' ' + cfg.color + ' border-current shadow-sm ring-2 ring-current/20' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>{cfg.emoji} {cfg.label}</button>;
                })}
              </div>
            </div>
            ...
            {availableVis.length > 1 && (
              <div>
                ...
                <div className="flex gap-1.5">
                  {availableVis.map(v => {
                    const vc = VISIBILITY_CONFIG[v];
                    return <button key={v} onClick={() => setNewEvt(p => ({ ...p, visibility: v }))} className={`px-3 py-1.5 text-xs font-bold rounded-xl border cursor-pointer active:scale-[0.98] transition-all flex items-center gap-1 ${newEvt.visibility === v ? vc.bgColor + ' ' + vc.color + ' border-current shadow-sm ring-2 ring-current/20' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>{vc.emoji} {vc.label}</button>;
                  })}
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <input value={newEvt.title} onChange={e => setNewEvt(p => ({ ...p, title: e.target.value }))} placeholder={'\uC77C\uC815 \uC81C\uBAA9\uC744 \uC785\uB825\uD558\uC138\uC694'} autoFocus className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" />
            </div>
            ...
            {/* Client + Desc */}
            <div><label className="text-xs font-bold text-slate-500 mb-1.5 block">{'\uAD00\uB828 \uC758\uB8B0\uC778'}</label><input value={newEvt.clientName} onChange={e => setNewEvt(p => ({ ...p, clientName: e.target.value }))} placeholder={'\uC120\uD0DD'} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-brand" /></div>
            <div><label className="text-xs font-bold text-slate-500 mb-1.5 block">{'\uBA54\uBAA8'}</label><textarea value={newEvt.description} onChange={e => setNewEvt(p => ({ ...p, description: e.target.value }))} placeholder={'\uC120\uD0DD'} rows={2} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-brand resize-none" /></div>
