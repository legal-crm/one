...
export default function LawyerProfileEditor({ lawyer, onSave, onClose, inline = false, onImageChange }: LawyerProfileEditorProps) {
  ...
  const sectionTitleCls = inline
    ? 'text-xs md:text-sm font-extrabold text-brand flex items-center gap-1.5'
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl overflow-hidden border border-brand/20 shadow-xs shrink-0">
              <img src={previewAvatar} alt={form.name} className="w-full h-full object-cover" />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
            <button
              onClick={() => setShowFullPreview(true)}
              className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 px-3.5 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer active:scale-[0.98]"
            >
              <Eye className="w-3.5 h-3.5" />
              <span className="whitespace-nowrap">전체 미리보기</span>
            </button>
            <button
              onClick={handleSave}
              className="flex items-center gap-1.5 bg-brand hover:bg-brand-hover text-white px-4 py-2 rounded-xl text-xs font-extrabold transition-all cursor-pointer active:scale-[0.98] shadow-xs"
            >
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className={sectionCls}>
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 shrink-0 shadow-xs">
                  <img src={previewAvatar} alt="프로필" className="w-full h-full object-cover" />
                </div>
            ...
            <div className={sectionCls}>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  {form.fields.length > 0 && (<div className="flex flex-wrap gap-1 mt-1.5">{form.fields.map(f => (<span key={f} className={tagCls}>#{f}</span>))}</div>)}
                </div>
                <div>
                  <label className={labelCls}>상세 전문 분야 (콤마로 구분)</label>
            ...
            <div className={sectionCls}>
              ...
              <div className="space-y-3">
                <div><label className={labelCls}>변호사 소개글</label><textarea className={`${inputCls} min-h-[80px] resize-y`} value={form.bio} onChange={e => updateForm({ bio: e.target.value })} placeholder="변호사 소개글을 입력하세요..." /></div>
                <div>
                  <label className={labelCls}>경력 사항</label>
                  <div className="space-y-1.5">
                    {(form.career || []).map((item, idx) => (
                      <div key={idx} className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => handleRemoveCareer(idx)} className="w-6 h-6 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-500 flex items-center justify-center transition-colors cursor-pointer"><Trash2 className="w-3 h-3" /></button>
                      </div>
                    ))}
                    <div className="flex gap-1.5 pt-0.5">
                      ...
                      <button onClick={handleAddCareer} className={addBtnCls}><Plus className="w-3 h-3" />추가</button>
                    </div>
                  </div>
                </div>
            ...
            <div className={sectionCls}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                <div><label className={labelCls}><span className="flex items-center gap-1"><Globe className="w-3 h-3 text-slate-500" />홈페이지</span></label><input className={inputCls} value={form.websiteUrl || ''} onChange={e => updateForm({ websiteUrl: e.target.value })} placeholder="https://..." /></div>
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-4 space-y-2.5">
              <div className="flex items-center justify-between bg-white border border-slate-200 px-3 py-2 rounded-xl shadow-xs">
                ...
                <div className="flex gap-1">
                  <button onClick={() => setPreviewTab('home')} className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-colors cursor-pointer ${previewTab === 'home' ? 'bg-brand text-white shadow-xs' : 'text-slate-500 hover:bg-slate-100'}`}>변호사홈</button>
                  <button onClick={() => setPreviewTab('info')} className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-colors cursor-pointer ${previewTab === 'info' ? 'bg-brand text-white shadow-xs' : 'text-slate-500 hover:bg-slate-100'}`}>상세정보</button>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
                ...
                <div className="relative bg-gradient-to-br from-slate-900 via-[#1e1b4b] to-[#1E3A5F] overflow-hidden">
                  ...
                  <div className="relative z-10 p-4 flex items-start gap-3.5">
                    <div className="relative shrink-0">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/20 shadow-lg"><img src={previewAvatar} alt={form.name} className="w-full h-full object-cover" /></div>
                  ...
                  {previewTab === 'home' && (
                    <div className="space-y-3 animate-fadeIn">
                      <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 space-y-1">
                        <h4 className="font-bold text-xs text-slate-900 flex items-center gap-1.5"><BookOpen className="w-3 h-3 text-[#1E3A5F]" />변호사 소개</h4>
                        <p className="text-xs text-slate-600 leading-relaxed font-medium line-clamp-3">{form.bio || '소개글을 입력해주세요.'}</p>
                      </div>
                      <div className="space-y-1.5">
        ...
        {/* 저장 토스트 */}
        {saveToast && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl shadow-xl flex items-center gap-2 text-xs font-bold animate-fadeIn z-[99999]">
