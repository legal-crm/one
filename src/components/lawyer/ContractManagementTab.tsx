        ...
        <table className="w-full">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              const cfg = (c && c.status && CONTRACT_STATUS_CONFIG[c.status]) || CONTRACT_STATUS_CONFIG.drafting;
              const dateObj = new Date(c.updatedAt || c.createdAt || Date.now());
              const dateStr = !isNaN(dateObj.getTime()) ? dateObj.toLocaleDateString('ko-KR') : '-';
              return (
                <tr key={c.id} className="border-t border-slate-100 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-3.5 text-slate-600 text-sm">{c.lawyerName || lawyerName}</td>
                  <td className="p-3.5 text-right font-bold text-sm text-slate-800">{(c.totalFee && c.totalFee > 0) ? <><span className="text-brand">{(c.totalFee * 10000).toLocaleString()}</span>원</> : '-'}</td>
                  <td className="p-3.5 text-center">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-lg ${cfg.bgColor} ${cfg.color}`}>{cfg.emoji} {cfg.label}</span>
                  </td>
                  <td className="p-3.5 text-sm text-slate-500">{dateStr}</td>
                  <td className="p-3.5 text-center">
